...
void yk_innit_vulkan(YkRenderere* renderer);
void yk_create_surface(YkRenderere* renderer);
void yk_pick_physdevice(YkRenderere* renderer);
void yk_find_queues(YkRenderere* renderer);
void yk_create_device(YkRenderere* renderer);
void yk_create_swapchain(YkRenderere* renderer);
void createDescriptorSetLayout(YkRenderere* renderer);
void yk_create_gfx_pipeline(YkRenderere* renderer);
void yk_create_cmd_pool(YkRenderere* renderer);
void yk_create_vert_buffer(YkRenderere* renderer);
void yk_create_index_buffer(YkRenderere* renderer);
void createUniformBuffers(YkRenderere* renderer);
void createDescriptorPool(YkRenderere* renderer);
void createDescriptorSets(YkRenderere* renderer);
void yk_create_cmd_buffer(YkRenderere* renderer);
void yk_create_sync_objs(YkRenderere* renderer);
b8 yk_recreate_swapchain(YkRenderere* renderer);

void copyBuffer(YkRenderere* renderer,VkBuffer srcBuffer, VkBuffer dstBuffer, VkDeviceSize size);
void updateUniformBuffer(YkRenderere* renderer, uint32_t currentImage);

void create_buffer(YkRenderere* ren, VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, VkBuffer* buffer, VkDeviceMemory* bufferMemory);


void yk_innit_renderer(YkRenderere* renderer, YkWindow* window)
{
    start_time = clock();
...
u32 findMemoryType(YkRenderere* renderer ,u32 typeFilter, VkMemoryPropertyFlags properties) {
